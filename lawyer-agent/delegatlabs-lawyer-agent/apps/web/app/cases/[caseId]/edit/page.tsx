...
import { getActiveCase, updateActiveCase, CASE_ROLE_SUGGESTIONS } from "../../../../lib/case-storage";
...
export default function EditCasePage() {
  ...
  useEffect(() => {
    ...
    const record = getActiveCase(caseId);
    if (record) {
      setCaseTitle(record.caseTitle);
      setCaseType(record.caseType);
      setCaseNumber(record.caseNumber || "");
      setCnrNumber(record.cnrNumber || "");
      setFirNumber(record.firNumber || "");
      setPoliceStation(record.policeStation || "");
      setSections(record.sections || "");
      setCourtType(record.courtType);
      setCourtName(record.courtName);
      setCourtNumber(record.courtNumber || "");
      setStage(record.stage);
      setPreviousDate(record.previousDate || "");
      setNextDate(record.nextDate || "");
      setFof(record.fof || "");
      setNextAction(record.nextAction || "");
      setPartyAName(record.partyA?.name || "");
      setPartyARole(record.partyA?.legalRole || "");
      setPartyBName(record.partyB?.name || "");
      setPartyBRole(record.partyB?.legalRole || "");
      setClientSide(record.clientParty || "partyA");
  ...
  // Auto-fill suggestions on Case Type changes
      ...
      <main className="flex-grow max-w-3xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-500">
            {lang === "hi" ? "मुकदमे के विवरण को संपादित करें।" : "Modify current active lawsuit details."}
          </p>
        </div>
